...
class S3Credentials(dict[str, S3Credential]):
    ...
    @classmethod
    def _convert(cls, data: S3Credentials | Path | dict[str, dict]) -> S3Credentials:
...
class S3Manager:
    ...
    def fetch(self, remote: RemoteFileLocation, dst: Path) -> None:
        if dst.exists():
    ...
    def upload(self, remote: RemoteFileLocation, src: Path) -> None:
        if src.is_dir():
